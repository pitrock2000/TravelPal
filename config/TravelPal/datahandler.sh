...
runMonitors() {
  ...
  for i in `seq 1 5`; do
    getCurrentData
    [ -e /mnt/sd_nav ] && echo $i >> "$HDSTOREDIR"/sdnavlog && sleep 1 && break
    sleep 1
  done
  setNAVIparams
  echo $NAVISTATUS >> "$HDSTOREDIR"/sdnavlog
...
takeaction() {
  ...
  if [ "${taction}" = "travlRd" ]; then
    ...
    [ $NAVISTATUS -eq 2 ] && rm -f "${SDSPDCAMDIR}/speedcam.spdb"
...
getCurrentData() {
  ...
  getGRTimeSec
  echo "currentData#${HEADING}#${DRVDIST}#${FUELEFC}#${DRVTIME}#${GPSSPEED}#${LENUNIT}#${GRTIMESEC}"
}
